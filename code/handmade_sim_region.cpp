...
BeginSim(memory_arena *SimArena, game_state *GameState, world *World, world_position Origin, rectangle3 Bounds, real32 dt) {
    ...
    // entities lager than max entity radius by adding them multiple times
...
MoveEntity(game_state *GameState, sim_region *SimRegion, sim_entity *Entity, real32 dt,
    ...
    for (uint32 Iteration = 0; Iteration < 4; ++Iteration) {
        ...
        if (PlayerDeltaLength > 0.0f) {
                     ...
                     TestHighEntityIndex++)
                {
                        ...
                        CanCollide(GameState, Entity, TestEntity))
                    {
                             ...
                             ++EntityVolumeIndex)
                        {
                            ...
                            for (uint32 TestEntityVolumeIndex = 0;
                                 TestEntityVolumeIndex < TestEntity->Collision->VolumeCount;
                                 ++TestEntityVolumeIndex)
                            {
                                sim_entity_collision_volume *TestVolume =
                                    TestEntity->Collision->Volumes + TestEntityVolumeIndex;
                                ...
                                if (Rel.z >= MinCorner.z && Rel.z < MaxCorner.z) {
                                    ...
                                    if (IsSet(TestEntity, EntityFlag_Traversable)) {
                                        ...
                                        // TODO: We need a conecpt of stepping onto vs. stepping
            ...
            v3 WallNormal;
            sim_entity *HitStopEntity;
            real32 tStop;
            if (tMin < tMax) {
                tStop = tMin;
                HitStopEntity = HitEntityMin;
                WallNormal = WallNormalMin;
            } else {
                tStop = tMax;
                HitStopEntity = HitEntityMax;
            ...
            if (HitStopEntity) {
                ...
                bool32 StopsOnCollision = HandleCollision(GameState, Entity, HitStopEntity);
                if (StopsOnCollision) {
    ...
    // TODO: Handle overlapping precisely by moving it into collision loop?
    {
        // TODO: Spatial partition here!
        for (uint32 TestHighEntityIndex = 0;
             TestHighEntityIndex < SimRegion->EntityCount;
