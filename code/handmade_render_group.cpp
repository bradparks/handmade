                     ...
                     loaded_bitmap *Texture, rectangle2i ClipRect, bool32 Even)
{
    ...
    if (HasArea(FillRect)) {
        ...
        uint32 RowAdvance = 2 * Buffer->Pitch;
...
#define Mi(a, i) ((int *)&(a))[i]
                ...
                __m128i WriteMask = _mm_castps_si128(_mm_and_ps(_mm_and_ps(_mm_cmpge_ps(U, Zero),
                                                                           _mm_cmple_ps(U, One)),
                                                                _mm_and_ps(_mm_cmpge_ps(U, Zero),
                                                                           _mm_cmple_ps(V, One))));
                ...
                //if (_mm_movemask_epi8(WriteMask))
                {
                    ...
                    __m128 Texelr = _mm_add_ps(_mm_mul_ps(l0, TexelAr), _mm_add_ps(_mm_mul_ps(l1, TexelBr), _mm_add_ps(_mm_mul_ps(l2, TexelCr), _mm_mul_ps(l3, TexelDr))));
                    __m128 Texelg = _mm_add_ps(_mm_mul_ps(l0, TexelAg), _mm_add_ps(_mm_mul_ps(l1, TexelBg), _mm_add_ps(_mm_mul_ps(l2, TexelCg), _mm_mul_ps(l3, TexelDg))));
                    __m128 Texelb = _mm_add_ps(_mm_mul_ps(l0, TexelAb), _mm_add_ps(_mm_mul_ps(l1, TexelBb), _mm_add_ps(_mm_mul_ps(l2, TexelCb), _mm_mul_ps(l3, TexelDb))));
                    __m128 Texela = _mm_add_ps(_mm_mul_ps(l0, TexelAa), _mm_add_ps(_mm_mul_ps(l1, TexelBa), _mm_add_ps(_mm_mul_ps(l2, TexelCa), _mm_mul_ps(l3, TexelDa))));
...
AllocateRenderGroup(memory_arena *Arena, uint32 MaxPushBufferSize,
    ...
    Result->Transform.FocalLength = 0.6f; // NOTE: Meters the person is sitting from their monitor
...
GetRenderEntityBasisP(render_transform *Transform, v3 OriginalP) {
    ...
    v3 RayXY = V3(P.xy, 1.0f);

    if (DistanceToPZ > NearClipPlane) {
        v3 ProjectedXY = (1.0f / DistanceToPZ) * Transform->FocalLength * RayXY;
...
{
#if 0
    entity_basis_p_result Basis = GetRenderEntityBasisP(Group, Entity->EntityBasis, ScreenDim);
    if (Basis->Valid) {
