        ...
        switch (Work->FinalizeOperation) {
            ...
            case FinalizeAsset_Font: {
                ...
                for (u32 GlyphIndex = 1; GlyphIndex < HHA->GlyphCount; ++GlyphIndex) {
                    ...
                    Assert(Glyph->UnicodeCodePoint < HHA->OnePastHigestCodePoint);
...
LoadBitmap(game_assets *Assets, bitmap_id ID, b32 Immediate) {
    ...
    if (ID.Value) {
        if ((AtomicCompareExchangeUInt32(&Asset->State, AssetState_Queued, AssetState_Unloaded) ==
             AssetState_Unloaded))
        {
...
LoadFont(game_assets *Assets, font_id ID, b32 Immediate) {
    // TODO: Merge all this boilerplate!!!
    asset *Asset = Assets->Assets + ID.Value;
    ...
    if (ID.Value) {
             ...
             AssetState_Unloaded))
        {
            ...
            if (Immediate || Task) {
                ...
                u32 UnicodeMapSize = sizeof(u16) * Info->OnePastHigestCodePoint;
...
GetGlyphFromCodePoint(hha_font *Info, loaded_font *Font, u32 CodePoint) {
    u32 Result = 0;
    if (CodePoint < Info->OnePastHigestCodePoint) {
